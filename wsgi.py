...
def initialize():
    db.drop_all()
    db.create_all()
    create_staff('bobby', 'bobpass', 'bobmail.com', 'Bob', 'Smith')
...
auth_cli = AppGroup('auth', help='User object commands') 
@auth_cli.command("login", help="Login" )
...
@test.command("staff", help="Run Staff unit tests")
...
@test.command("student", help="Run Student unit tests")
@click.argument("type", default="unit")
def user_tests_command(type):
    if type == "unit":
...
@test.command("review", help="Run Review unit tests")
@click.argument("type", default="unit")
def user_tests_command(type):
    if type == "unit":
...
@click.argument("type", default="int")
def user_tests_command(type):
    if type == "int":
